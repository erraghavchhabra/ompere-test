...
function MachineryValuationWizardInner() {
  ...
  const { data: hourOptions, loading: loadingHours } =
    useFetchOptions<HourOption>(API.hours);
  const { data: yearOptions, loading: loadingYears } =
    useFetchOptions<YearOption>(API.years);
  const { data: engineOptions, loading: loadingEngines } =
    useFetchOptions<EngineOption>(API.engineConditions);
  ...
  const getMarginLow = (): number => toNum(settings.margin_low) ?? 0.8;

  // ── Helpers to look up factor from selected value ─────────────────────────
  const getHourFactor = (): number | null => {
    if (!selectedHours) return null;
    const found = hourOptions.find(
      (h) => String(h.id) === String(selectedHours),
    );
  ...
  const getYearFactor = (): number | null => {
    ...
    const found = yearOptions.find(
      (y) => String(y.name) === String(selectedYear),
    );
    return found != null ? toNum(found.factor) : null;
  };
  ...
  const getEngineFactor = (): number | null => {
    ...
    const found = engineOptions.find(
      (e) => String(e.id) === String(selectedEngineCondition),
    );
    return found != null ? toNum(found.factor) : null;
  };
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (selectedHours) {
        const hoursLabel =
          hourOptions.find((h) => String(h.id) === String(selectedHours))
            ?.name ?? selectedHours;
  ...
  const computePriceRange = (): { high: number; low: number } | null => {
    ...
    const yearFactorMin =
      yearSelected && yearFactor != null ? yearFactor : 0.58;
    const hoursFactorMax =
      hoursSelected && hourFactor != null ? hourFactor : 1.0;
    const hoursFactorMin =
      hoursSelected && hourFactor != null ? hourFactor : 0.87;
    const engineFactors = engineOptions
      .map((e) => toNum(e.factor))
      .filter((f): f is number => f !== null);
    const engineFactorMax =
      engineSelected && engineFactor != null
        ? engineFactor
        : engineFactors.length
          ? Math.max(...engineFactors)
          : 1.0;
    const engineFactorMin =
      engineSelected && engineFactor != null ? engineFactor : 0.95;
  ...
  const buildFactorRows = (): FactorRow[] => {
    ...
    rows.push(
      ...
      },
      {
        ...
        maxVal: engineSelected
          ? engineFactor
          : engineOptions.length
            ? Math.max(...engineOptions.map((e) => toNum(e.factor) ?? 0))
            : 1.05,
        minVal: engineSelected ? engineFactor : 0.95,
    ...
    <section className="min-h-screen py-14 bg-gradient-to-b from-white to-orange-50/40 relative overflow-hidden">
      <div className="max-w-7xl mx-auto px-6">
        <div className="grid lg:grid-cols-2 gap-8 items-stretch">
          ...
          <div className="flex flex-col">
              ...
              {["Machine Details", "Images"].map((label, index) => {
                ...
                return (
                  <div key={label} className="flex items-center flex-1">
                    <div
                      className={`w-10 h-10 flex-shrink-0 rounded-full flex items-center justify-center text-sm font-semibold transition ${
                        step === current
                    ...
                    {current < 2 && (
                      <div className="flex-1 h-[2px] bg-orange-200 mx-2 sm:mx-4" />
                    )}
                  </div>
                );
              })}
            ...
            {/* Main Card */}
            <div className="bg-white rounded-3xl p-4 md:p-8 border border-orange-100 shadow-sm flex-1">
              {/* ── STEP 1 ── */}
              {step === 1 && (
                <>
                  <h2 className="text-2xl font-bold text-[#1a1a1a] mb-6">
                    Machine Details & Condition
                  </h2>
                  <div className="space-y-5">
                    ...
                    <div>
                      ...
                      {loadingHours ? (
                        ...
                        <div className="relative">
                          {isOptionalEmpty(selectedHours) && (
                            <ChevronDown className="bounce-arrow absolute -top-5 right-6 w-4 h-4 text-[#f07020]" />
                          )}
                          <select
                            value={selectedHours}
          ...
          {/* RIGHT SIDE */}
          <div className="bg-white rounded-3xl p-4 md:p-8 border border-orange-100 shadow-sm h-full flex flex-col gap-6">
            {/* PRICE RANGE CARD */}
            <div className="bg-gradient-to-b from-white to-orange-50/40 rounded-3xl p-4 md:p-8 border border-orange-100">
      ...
      <style jsx>{`
        @keyframes pulseGlow {
          0%,
          100% {
            box-shadow: 0 0 0 0 rgba(240, 112, 32, 0.4);
          }
          50% {
